...
let
  ...
  mkStaticMapping = pluginDefault: actionDescription:
  ...
  mkValuedMapping = pluginDefault: actionDescription:
    helpers.defaultNullOpts.mkNullableWithRaw' {
      type = lib.types.either
        (lib.types.either (lib.types.submodule valuedMappingType) lib.types.str)
        lib.types.bool;
      inherit pluginDefault;
      description = ''
        Sets a global mapping to Neovim, which allows you to ${actionDescription}.
  ...
  settingsOptions = with defaultNullOpts; {
    ...
    mappings = mkSettingsOption {
      options = {
        ...
        toggleLeftSide =
          mkStaticMapping "<Leader>nql" "toggle the left side buffer";
        toggleRightSide =
          mkStaticMapping "<Leader>nql" "toggle the right side buffer";
        widthUp =
          mkValuedMapping "<Leader>n=" "increase the width of the main window";
        widthDown =
          mkValuedMapping "<Leader>n-" "decrease the width of the main window";
        scratchPad =
          mkStaticMapping "<Leader>ns" "toggle the scratchPad feature";
      };
      description =
    ...
    buffer = let
      mkNormalizedInt' = args:
        mkNullableWithRaw'
        (args // { type = (lib.types.ints.between (-1) 1); });
      mkNormalizedInt = pluginDefault: description:
        mkNormalizedInt' { inherit pluginDefault description; };
    ...
    integrations = let
      mkTreeOption = name:
        { position ? "left", reopen ? true }:
        mkSettingsOption {
          options = {
            position =
              mkEnum [ "left" "right" ] position "The position of the tree.";
      ...
      leftTrees = lib.attrsets.genAttrs [ "NvimTree" "NeoTree" "undotree" ]
        (name: mkTreeOption name { });

      rightTrees = lib.attrsets.genAttrs [ "neotest" "TSPlayground" "outline" ]
