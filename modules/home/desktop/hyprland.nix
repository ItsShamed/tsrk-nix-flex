    ...
    tsrk.hyprland = {
      ...
      backgrounds = {
        lockscreen = lib.options.mkOption {
          description =
            "Image to use as the lockscreen background (needs Darkman)";
          type = lib.types.path;
        ...
        bindd = [
          ...
          "$mainMod SHIFT, E, Show Power Menu, exec, ${
            if config.programs.rofi.enable then
              "${config.programs.rofi.finalPackage}/bin/rofi -show p -modi p:'rofi-power-menu --logout \"loginctl terminate-session\"'"
            else
